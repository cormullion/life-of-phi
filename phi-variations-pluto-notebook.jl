...
begin
	...
	fontsize(26)
	fontface("JuliaMono")
	table = Table(fill(1100/(1 + length(fontlist)), 1 + length(fontlist)), 	[150, 150,150,400])
...
There are about 20 different versions of the phi character in the Unicode tables, but the three main versions used in computing are:
...
These designs are from the JuliaMono font. 

Here are the same glyphs shown in Pluto's default text font (Alegreya Sans, unless modified by the browser).

- 03D5: ϕ 
- 03C6: φ 
- 0278: ɸ 

Remember that font fallbacks are used in browsers: ie, if the required font doesn't have a glyph for a character, then other fonts are searched until a suitable glyph is found. So because Alegreya doesn't have anything for U+0278, the browser shows something similar. (On my system I get U+03C6 from Helvetica here!)

"

# ╔═╡ f693c61e-5c5e-4e09-9252-49a35b8f5ba3
md" In Julia, the Golden Ratio Phi, 1.618... is available as the constant `MathConstants.golden`. It's known as the `varphi` version of phi, and you can type it in the REPL with the LATEX completion `\\varphi`. The Unicode character code of this is `03C6`"
...
The \\varphi (U+03C6) version of Phi is called \"the loopy\" version in the Unicode documentation:"

# ╔═╡ bb0d519b-45ea-4640-b0f2-adc4498f3b21
md" > For mathematical and technical use, the straight form of the small phi is an important symbol (TEX `\phi` U+03D5) and needs to be consistently distinguishable from the loopy form (TEX `\varphi` U+03C6). The straight form phi glyph at U+03D5 is used as the representative glyph for the phi symbol at U+03D5 to satisfy this distinction. 
...
The appearance of this image depends a lot on which fonts you have installed and are active on your computer.

Luxor/Cairo *doesn't* invoke the fallback mehanism that you see (but might not notice) in browsers and editors, so if a font doesn't have the character, you'll see a box or something similar, rather than a replacement from another font. Typically, you'll see a Helvetica substitute font if the font can't be found on your computer.

If you have Consolas installed, you'll notice that the font has swapped the \\phi and \\varphi characters. This is a well-known issue with this font, and a few others.

Also note that the Alegreya Sans font used in Pluto uses the same (loopy) glyph for both U+03D5 and U+03C6.
...
manifest_format = "2.0"
project_hash = "a9fea0f2f2470bb07a3627d8c8c29985b13221f2"
